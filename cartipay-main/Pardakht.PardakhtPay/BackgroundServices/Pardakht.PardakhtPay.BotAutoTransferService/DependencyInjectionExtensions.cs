...
using Pardakht.PardakhtPay.BotAutoTransferService.CheckManagers;
...
namespace Pardakht.PardakhtPay.BotAutoTransferService
{
    public static class DependencyInjectionExtensions
    {
        public static void AddDependencyInjections(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            #region Time Zone
            services.AddSingleton(typeof(ITimeZoneService), typeof(TimeZoneService));
            #endregion
            ...
            var bankBotConfiguration = provider.GetRequiredService<IOptions<BankBotConfiguration>>();
            ...
            {
                services.AddScoped(typeof(IBankBotService), typeof(Application.Services.BankBotService));
            }
            services.AddTransient(typeof(ITransactionQueueService), typeof(TransactionAzureQueueService));

            services.AddScoped(typeof(CurrentUser), typeof(CurrentUser));

            //services.AddScoped(typeof(ITenantCacheService), typeof(TenantCacheService));
            ...
            services.AddSingleton(typeof(IOperationManager), typeof(OperationManager));
            services.AddTransient(typeof(IAutoTransferOperationManager), typeof(AutoTransferOperationManager));
            services.AddTransient(typeof(ICheckTransferManager), typeof(CheckTransferManager));

            services.AddTransient(typeof(IWithdrawalOperationManager), typeof(WithdrawalOperationManager));
            services.AddTransient(typeof(ICheckWithdrawalManager), typeof(CheckWithdrawalManager));

            services.AddTransient(typeof(IManualTransferOperationManager), typeof(ManualTransferOperationManager));
            services.AddTransient(typeof(ICheckManualTransferManager), typeof(CheckManualTransferManager));

            //services.AddScoped(typeof(ITenantApiRepository), typeof(TenantApiRepository));
            ...
            services.AddScoped(typeof(ICheckPausedAccountsManager), typeof(CheckPausedAccountsManager));

            services.AddScoped(typeof(ICheckCardNumberManager), typeof(CheckCardNumberManager));

            services.AddScoped(typeof(ICardHolderNameOperationManager), typeof(CardHolderNameOperationManager));

            services.AddScoped(typeof(IInvoiceOperationManager), typeof(InvoiceOperationManager));

            services.AddScoped(typeof(IPardakhtPalOperationManager), typeof(PardakhtPalOperationManager));

            services.AddScoped(typeof(IProxyPaymentThresholdManager), typeof(ProxyPaymentThresholdManager));

            services.AddScoped(typeof(ICheckLoginDeviceStatusManager), typeof(CheckLoginDeviceStatusManager));

            services.RegisterGenericServices<TenantManagementSettings>(configuration);
            services.RegisterGenericServices<PardakhtPayAuthenticationSettings>(configuration);
            //services.RegisterGenericServices<TimeZoneConfiguration>(configuration);
            //services.AddTransient(typeof(IUniversalApiService), typeof(UniversalApiService));
            ...
            services.AddSingleton(typeof(ISmsService), typeof(SmsMockService));

            services.AddHttpClient();

            services.AddScoped<UserContext>();

            services.AddHostedService<OperationManager>();
