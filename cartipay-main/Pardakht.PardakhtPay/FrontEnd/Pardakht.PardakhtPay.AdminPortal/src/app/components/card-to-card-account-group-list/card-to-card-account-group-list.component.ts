...
@Component({
  selector: 'app-card-to-card-account-group-list',
  templateUrl: './card-to-card-account-group-list.component.html',
    styleUrls: ['./card-to-card-account-group-list.component.scss'],
    animations: fuseAnimations
})
export class CardToCardAccountGroupListComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.columnDefs = [
            ...
            {
                headerName: this.translateService.instant('MERCHANT.LIST-COLUMNS.CARD-TO-CARD-ACCOUNT'),
                field: "accounts",
                resizable: true,
                sortable: true,
                width: 600,
        ...
        this.deleteSuccess$ = this.store.select(coreState.getCardToCardAccountGroupDeleteSuccess);
        ...
        this.deleteError$ = this.store.select(coreState.getCardToCardAccountGroupDeleteError);
    ...
    delete(id: number) {
        if (id) {
            this.store.dispatch(new cardToCardAccountGroupActions.Delete(id));
        }
    }
    ...
    onCellClicked(e: CellClickedEvent) {
        if (e && e.colDef && e.colDef.field != 'id') {
            ...
            if (selectedRows.length > 0) {
                ...
                this.ngZone.run(() => this.router.navigate(['/cardtocardaccountgroup/' + selected.id]));
            }
        }
    }
