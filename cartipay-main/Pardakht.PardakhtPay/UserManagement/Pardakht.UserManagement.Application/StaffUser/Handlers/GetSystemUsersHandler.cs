...
{
    public class GetSystemUsersHandler : StaffUserHandlerBase
    {
        public GetSystemUsersHandler(StaffUserHandlerArgs handlerArgs) :base(handlerArgs){ }
        ...
        public async Task<WebResponse<IEnumerable<Shared.Models.WebService.StaffUser>>> Handle(string platformGuid, string tenantGuid)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(tenantGuid))
                {
                    ...
                    if (userContext.IsProviderUser())
                    {
                        ...
                        result = await staffUserManager.GetProviderUsersByUserTypeAsync(platformGuid, UserType.ApiUser).ConfigureAwait(false);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (userContext.IsProviderUser())
                    {
                        ...
                        result = await staffUserManager.GetTenantUsersByUserTypeAsync(platformGuid, tenant.Id, true, null, tenantGuid, UserType.ApiUser).ConfigureAwait(false);
                    }
                    else
                    {
                        ...
                        result = await staffUserManager.GetTenantUsersByUserTypeAsync(platformGuid, tenant.Id, allOwners, parentAccountId, tenantGuid, UserType.ApiUser).ConfigureAwait(false);
                    }
                }
            ...
            {
                logger.LogError($"GetSystemUsersHandler.Handle: an error occurred getting staff users for platform {platformGuid}, tenant {tenantGuid} - {ex}");
                throw;
            }
        }
