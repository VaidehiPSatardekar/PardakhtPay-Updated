...
{
    public class ForgotPasswordByUsernameHandler : StaffUserHandlerBase
    {
        private readonly IAuditLogManager auditLogManager;
        public ForgotPasswordByUsernameHandler( IAuditLogManager auditLogManager, StaffUserHandlerArgs handlerArgs) : base( handlerArgs)
        {
            this.auditLogManager = auditLogManager;
        }

        public async Task<WebResponse> Handle(ForgotResetByUsernameRequest request)
        {
            try
            {
                var staffUser = await staffUserManager.GetByUsername(request.Username);
                if (staffUser == null)
                {
                    throw new Exception($"Staff user {request.Username} not found");
                }
            ...
            {
                logger.LogError($"ForgotPasswordHandler.Handle: an error occurred resetting staff user password {request.Username} - {ex}");
                throw;
            }
        }
    }
