...
{
    public class GetStaffUsersHandler : StaffUserHandlerBase
    {
        public GetStaffUsersHandler(StaffUserHandlerArgs handlerArgs) : base(handlerArgs) { }

        public async Task<WebResponse<IEnumerable<Shared.Models.WebService.StaffUser>>> Handle(string platformGuid, string tenantGuid, int? brandId)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(tenantGuid))
                {
                    ...
                    if (userContext.IsProviderUser())
                    {
                        ...
                        result = await staffUserManager.GetProviderUsers(platformGuid).ConfigureAwait(false);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (userContext.IsProviderUser())
                    {
                        ...
                        result = await staffUserManager.GetTenantUsersByUserTypeAsync(platformGuid, tenant.Id, true, null, tenantGuid, UserType.StaffUser, brandId).ConfigureAwait(false);
                    }
                    else
                    {
                        ...
                        result = await staffUserManager.GetTenantUsersByUserTypeAsync(platformGuid, tenant.Id, allOwners, parentAccountId, tenantGuid, UserType.StaffUser, brandId).ConfigureAwait(false);
                    }
                }
            ...
            {
                logger.LogError($"GetStaffUsersHandler.Handle: an error occurred getting staff users for platform {platformGuid}, tenant {tenantGuid} - {ex}");
                throw;
            }
        }
    }
